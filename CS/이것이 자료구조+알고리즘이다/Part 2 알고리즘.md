...
- 더 나아갈 길이 보이지 않을 때까지 깊이 들어가는 방법이다.
- 최대한 안까지 깊이 들어갔다가 막다른길이 나타나면, 다시 뒤로 돌아가 다른 경로로 뻗어 있는 정점을 타고 방문을 재개하는 방식으로 동작한다. 마치 미로찾기와 같다.
    - 방문한 정점에 방문 체크를 하고 인접한 노드 중 방문하지 않은 노드로 접속한다.
...
- 꼼꼼하게 좌우를 모두 살피며 나아가는 방법이다.
- 시작 후 깊이가 1인 모든 노드를 탐색하고, 다음은 깊이가 2인 모든 노드를 탐색하는 방식이다.
...
- 위상이란, 어떤 정점이 다른 정점과의 관계 속에서 가지는 위치를 말한다.
- 위상을 정렬하기 위해선 첫째로 그래프에 방향성이 있고, 둘째로 그래프 내에[ 사이클이 없어야 한다.
    - 이런 그래프를 DAG라고 한다.
- 위상 정렬이란 간선을 뻗어내는 정점인 앞, 받아내는 정점인 뒤의 관계를 차근차근 정렬하는 것이다.
    - 즉 진입 간선 → 정점 → 진출 간선이 된다.
- 간선과 정점을 하나씩 지워가며 가장 뒤에있는 것부터 천천히 연결리스트에 담아 정렬시킨다.

## 최소 신장 트리

- 정점과 간선이 있고, 간선에 가중치라는 새로운 속성이 있을 수 있다.
- 가중치가 있다면 그래프로 표현할 수 있는 요소가 훨씬 다양해진다. 지도상에서 도시와 도로, 도로의 거리를 가중치로 표현할 수 있게 된다.
- 신장 트리(Spanning Tree)의 신장이란 떨어져있는 둘 사이를 다리 등으로 연결하다는 의미이다. 즉 신장 트리는 그래프트의 모든 정점을 연결하는 트리이다.
- 최소 신장 트리란, 최소 가중치 신장 트리라고도 불린다. 여러 간선 중 가중치의 합이 최소가 되는 간선만 남긴 신장 트리가 바로 최소 신장 트리이다.
    - 즉 그래프에서 신장이 짧은 것만 남겨놓고, 긴 간선은 모두 제거해버려 트리로 만드는 것이다.
- 최소한의 비용으로 모든 도시를 연결하는 도로를 건설할 방법을 찾는데에 아주 좋은 방법이다.
- 최소 신장 트리를 만드는 방법은 두 가지가 있다.

### 프림 알고리즘

- 최소 신장 트리에 연결된 정점들의 주변 정보를 파악해 최소 신장 트리를 완성해나가는 방식이다.
- 그래프와 최소 신장 트리를 준비한다. 이 때 최소 신장트리는 당연히 노드가 하나도 없다.
- 그래프에서 임의의 정점을 시작으로 선택, 최소 신장 트리의 뿌리 노드로 삽입한다.
- 인접한 간선의 가중치를 조사하여, 작은걸 골라 최소 신장 트리에 삽입한다. 단, 새로 삽입하는 정점은 사이클을 형성해서는 안된다.
- 모두 반복하면 종료된다.

### 크루스칼 알고리즘

- 그래프 내 모든 간선의 가중치 정보를 사전에 파악하고, 이 정보를 토대로 구축해나간다.
- 그래프 내의 모든 간선을 가중치의 오름차순으로 정렬해 목록을 만든다.
- 그 목록을 차례대로 순회, 간선을 최소 신장 트리에 추가한다. 이 때 신장 트리 내에 사이클이 형성되면 안된다.
- 알고리즘은 간단해보이나, 최소 신장 트리에 생기는 사이클을 효율적으로 감지하는 방법이 어렵다.

## 최단 경로 탐색 : 다익스트라 알고리즘 (Dijkstra)

- 프림 알고리즘과 유사하나, 경로의 길이를 감안해서 간선을 연결하는 점이 다르다.
    - 또한 사이클이 없는 방향성 그래프에 한해서만 사용할 수 있다.
- 시작 정점으로부터 모든 정점까지의 거리를 무한으로 세팅한 배열을 별도로 둔다.
- 시작 정점부터 인접한 경로의 길이를 모두 배열에서 갱신한다. 즉, 최단 경로를 계속해서 갱신해나가는 것이다.
- 이후 연결된 정점 중 짧은 노드에 방문하여 또 다시 최단 경로를 갱신한다. 이를 반복한다.

# Chapter 10 : 문자열 탐색

## 문자열 탐색 알고리즘의 개요

- 누구나 컴퓨터를 이용하는 사람이라면 Ctrl + F 단축키로 수많은 텍스트들의 뭉치에서 원하는 텍스트를 검색해본 적이 있을 것이다.
- 문자열 탐색은 간단해보이지만 이걸 구현하는 일은 독특한 재미를 가지고 있다.

## 고지식한 탐색 알고리즘

- 본문의 앞에서부터 끝까지 차근차근 탐색하는 알고리즘이다.
- 탐색하고자 하는 문자열을 배열에 담아, 배열을 한 칸씩 오른쪽으로 옮기며 비교하는 방식이다.
- 이렇게 우직하게 일하는 브루트 포스 알고리즘은, 패턴을 찾기 위해 최악의 경우 N x M 번의 비교를 수행한다.
- 이런 식으로 문자열 탐색을 구현한다면 리뷰 테러를 당할지도 모른다.

## 카프-라빈 알고리즘

- 리처드 카프와 마이클 라빈이 만든 해시 활용의 문자열 탐색 알고리즘이다.
- 패턴의 해시값과 본문 내에 있는 하위 문자열의 해시값만 비교하는 방식이다.
- 그러나 이는 브루트포스와 크게 달라보이지 않는다. 일일히 하나씩 비교해서 해싱을 해야하기 때문에, 해싱의 비용까지 생각하면 겉보기에 오히려 더 비싸보인다.
- 그러나 카프-라빈 알고리즘의 진가는 해시 함수의 비용을 획기적으로 줄이는 방법에 있다.
    - 이전 문자열에서 해싱한 결과를 가지고, 다음 문자열의 해싱값 계산을 도울 수 있다.
    - 마치 다이나믹 프로그래밍과 비슷하다.
- 이렇게 모든 문자열을 돌며 해싱을 빠르게 진행하고, 해시 값이 같다면 실제로 문자열이 같은지 한 번 더 검증한다. 이는 해시 충돌을 방지하기 위함이다.

## KMP 알고리즘

- KMP 역시 브루트포스처럼 왼쪽부터 오른쪽으로 쭉 옮겨가지만, 이 알고리즘은 비교할 필요가 없는 부분은 지나치고, 비교가 필요한 부분만 비교를 수행하는 똑똑한 알고리즘이다.
- 본문 내 문자열을 비교하고 나서, 다음 단계 탐색을 하기 전 최대한 이전의 탐색 정보를 활용하는 데에 포커싱을 두었다.
- 어느 문자열이든 접두부와 접미부가 있다.
    - 여기서 접두부와 접미부가 일치하는 순간이 있을 수 있는데, 이를 경계라고 한다.
    - 겹치는 부분에서 접두어가 같아질 때까지, 그 길이에서 접두어의 길이를 뺀 만큼 이동한다.
- 그러나 경계를 찾는 데에 소요되는 비용도 분명 있을 것이다.

## 보이어-무어 알고리즘

- 독특하게도 오른쪽에서 왼쪽으로 진행하는 방식이다. 하지만 이동은 왼쪽에서 오른쪽으로 간다.
- 또한, 대부분의 문자열 탐색 알고리즘이 이 알고리즘이다.
    - 오른쪽부터 비교를 해서, 원하는 패턴 안에 없는 문자가 있다면, 그 문자가 없는 곳으로, 패턴의 길이만큼 이동해버린다.
- 보이어-무어에는 나쁜 문자 이동과 착한 접미부 이동이 있다.
    - 나쁜 문자 이동은 나쁘다는 뜻이 아니라, 나쁜 문자를 이동시킨다는 뜻이다. 여기서 나쁜 문자는 본문과 패턴 사이를 이간질하는 본문의 문자이다.
    - 또한 착한 접미부 이동을 통해 잘 어울리는 문자열의 위치까지 한 번에 이동시킬 수 있다.